...
def _basic_auth_ok(credentials: HTTPBasicCredentials | None) -> bool:
    return bool(credentials and secrets.compare_digest(credentials.username.encode(), settings.app_username.encode()) and secrets.compare_digest(credentials.password.encode(), settings.app_password.encode()))
...
def authorize_run(request: Request, credentials: HTTPBasicCredentials | None = Depends(optional_security)) -> str:
...
def _is_operating_company_asset(asset: dict[str, Any]) -> bool:
    return not bool(NON_OPERATING_INSTRUMENT_RE.search(str(asset.get("name") or "")))
...
def _classify_news(articles: list[dict[str, Any]]) -> tuple[str, str, list[str]]:
    if not articles:
        return "U", "No company-specific news was retained at the point-in-time evidence cutoff.", ["no_news"]
    text = " ".join(" ".join(str(article.get(key) or "") for key in ("headline", "summary")).lower() for article in articles)
    ...
    return "U", "News exists, but the catalyst is not confidently classified by the point-in-time rules.", sorted(set(risk_flags))


def _score_candidate(*, drop_pct: float, prev_dollar_volume: float | None, spread_pct: float | None, catalyst_class: str, headline_count: int) -> int:
    """Legacy heuristic retained only for historical comparison/audit."""
...
def _model_triage(catalyst_class: str, model: dict[str, Any]) -> str:
    if model.get("hard_veto") and catalyst_class == "E":
        return "PASS / EXISTENTIAL RISK"
    if model.get("hard_veto") or float(model.get("damage_risk") or 0) >= 70:
        return "PASS / STRUCTURAL RISK"
    if model.get("verdict") == "INVESTIGATE":
        return "INVESTIGATE NOW"
    if model.get("verdict") == "WATCH":
        return "REVIEW"
    return "LOW PRIORITY"
...
async def _fetch_news_map(client: AlpacaClient, symbols: list[str], *, end_at: datetime) -> tuple[dict[str, list[dict[str, Any]]], int]:
    start = end_at - timedelta(hours=NEWS_LOOKBACK_HOURS)
    ...
    for batch in _chunks(symbols, NEWS_SYMBOL_BATCH_SIZE):
        ...
        for _ in range(NEWS_MAX_PAGES):
            ...
            params: dict[str, Any] = {"symbols": ",".join(batch), "start": start.isoformat().replace("+00:00", "Z"), "end": end_at.isoformat().replace("+00:00", "Z"), "sort": "desc", "limit": 50, "include_content": "false"}
            if token:
            ...
            for article in articles:
                if not isinstance(article, dict):
                    continue
                published = _parse_ts(article.get("created_at"))
                if published is None or published > end_at:
                    continue
                article_symbols = {str(s).upper() for s in article.get("symbols", []) if s}
                compact = {"id": article.get("id"), "headline": article.get("headline"), "summary": article.get("summary"), "source": article.get("source"), "created_at": article.get("created_at"), "updated_at": article.get("updated_at"), "url": article.get("url"), "symbols": sorted(article_symbols)}
...
def _create_scan(trigger_source: str, min_drop_pct: float, candidate_limit: int) -> UUID:
    with connection() as conn:
        with conn.cursor() as cur:
            cur.execute("INSERT INTO or_scans(trigger_source,scan_date,min_drop_pct,candidate_limit,status) VALUES (%s,%s,%s,%s,'running') RETURNING id", (trigger_source, datetime.now(LONDON).date(), min_drop_pct, candidate_limit))
...
def _existing_scheduled_scan() -> dict[str, Any] | None:
    with connection() as conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id,status,started_at,completed_at FROM or_scans WHERE scan_date=%s AND trigger_source='scheduled' AND status IN ('running','completed') ORDER BY started_at DESC LIMIT 1", (datetime.now(LONDON).date(),))
            row = cur.fetchone()
        conn.rollback()
    return row
...
def _extract_candidate(symbol: str, snapshot: dict[str, Any], asset: dict[str, Any], min_drop_pct: float) -> dict[str, Any] | None:
    previous, latest_trade, latest_quote = snapshot.get("prevDailyBar") or {}, snapshot.get("latestTrade") or {}, snapshot.get("latestQuote") or {}
    minute_bar, daily_bar = snapshot.get("minuteBar") or {}, snapshot.get("dailyBar") or {}
    ...
    if prev_dollar_volume is not None and prev_dollar_volume < MIN_PREV_DOLLAR_VOLUME:
        return None
    bid, ask = _number(latest_quote.get("bp")), _number(latest_quote.get("ap"))
    spread_pct = ((ask - bid) / ((ask + bid) / 2)) * 100.0 if bid and ask and ask >= bid and ((ask + bid) / 2) > 0 else None
    return {"symbol": symbol, "name": asset.get("name"), "exchange": asset.get("exchange"), "prev_close": prev_close, "last_price": last_price, "drop_pct": drop_pct, "prev_volume": prev_volume, "prev_dollar_volume": prev_dollar_volume, "bid": bid, "ask": ask, "spread_pct": spread_pct, "latest_trade_ts": latest_trade.get("t"), "raw_snapshot": snapshot}


async def execute_scan(scan_id: UUID, *, min_drop_pct: float = DEFAULT_MIN_DROP_PCT, candidate_limit: int = DEFAULT_CANDIDATE_LIMIT) -> None:
    async with _scan_lock:
        try:
            async with AlpacaClient(target_rpm=3000, max_retries=4, backoff_seconds=0.8) as client:
                ...
                eligible_assets = [asset for asset in assets if asset.get("tradable") is True and str(asset.get("asset_class") or "").lower() == "us_equity" and "otc" not in str(asset.get("exchange") or "").lower() and _is_operating_company_asset(asset) and re.fullmatch(r"[A-Z][A-Z0-9.\-]{0,9}", str(asset.get("symbol") or "").upper())]
                ...
                raw_candidates = [candidate for symbol, snapshot in snapshots.items() if symbol in asset_by_symbol and isinstance(snapshot, dict) if (candidate := _extract_candidate(symbol, snapshot, asset_by_symbol[symbol], min_drop_pct)) is not None]
                raw_candidates.sort(key=lambda item: item["drop_pct"])
                raw_candidates = raw_candidates[:candidate_limit]
                news_map, news_requests = await _fetch_news_map(client, [item["symbol"] for item in raw_candidates], end_at=evidence_cutoff)
                ...
                with conn.cursor() as cur:
                    for rank, item in enumerate(enriched, 1):
                        cur.execute(
                            ...
                            RETURNING id
                            """,
                            (scan_id, rank, item["symbol"], item["name"], item["exchange"], item["prev_close"], item["last_price"], item["drop_pct"], item["prev_volume"], item["prev_dollar_volume"], item["bid"], item["ask"], item["spread_pct"], item["latest_trade_ts"], item["catalyst_class"], item["catalyst_summary"], item["risk_flags"], item["headline_count"], Jsonb(item["headlines"]), item["heuristic_score"], item["triage_label"], Jsonb(item["raw_snapshot"])),
                        )
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                SELECT c.id,c.rank,c.symbol,c.name,c.exchange,c.prev_close,c.last_price,c.drop_pct,
                       ...
                       mr.setup_score,mr.catalyst_score,mr.resilience_score,mr.confirmation_score,
                       mr.damage_risk,mr.evidence_confidence,mr.core_score,mr.confidence_adjusted_score,
                       mr.damage_penalty,mr.damage_cap,mr.verdict AS model_verdict,mr.hard_veto,mr.hard_veto_reason,
                       mr.missing_inputs,mr.catalyst_analysis,mr.calculation_trace,mr.explanation,
                       mr.scoring_model_version,mr.scoring_config_version,mr.catalyst_prompt_version,
                       mr.catalyst_schema_version,mr.calibration_model_version,mr.target_definition,
                       es.id AS evidence_snapshot_id,es.signal_timestamp,es.evidence_cutoff,es.signal_price,
                       es.sector_hint,es.news_items AS evidence_news,es.technical_inputs,es.source_quality,es.snapshot_hash
                FROM or_candidates c
                LEFT JOIN LATERAL (
                    SELECT * FROM or_model_runs x WHERE x.candidate_id=c.id AND x.run_kind='original' ORDER BY x.created_at ASC,x.id ASC LIMIT 1
                ) mr ON true
                LEFT JOIN or_evidence_snapshots es ON es.id=mr.evidence_snapshot_id
                WHERE c.scan_id=%s ORDER BY c.rank
                """,
                (scan_id,),
            )
...
async def run_scan(background_tasks: BackgroundTasks, request: Request, scheduled: bool = Query(False), background: bool = Query(True), min_drop_pct: float = Query(DEFAULT_MIN_DROP_PCT, ge=5, le=90), candidate_limit: int = Query(DEFAULT_CANDIDATE_LIMIT, ge=1, le=MAX_CANDIDATE_LIMIT), auth_mode: str = Depends(authorize_run)) -> dict[str, Any]:
    if scheduled:
...
def update_candidate(candidate_id: int, payload: dict[str, Any], _: str = Depends(require_basic)) -> dict[str, Any]:
    ...
    with connection() as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE or_candidates SET decision=%s,review_notes=%s,reviewed_at=now() WHERE id=%s RETURNING id,decision,review_notes,reviewed_at", (decision, review_notes, candidate_id))
            row = cur.fetchone()
            if not row:
